...
async function fetchData(searchText: string, perPage: number): Promise<PhotosResponse> {
  const response = searchText.trim()
    ...
    : await fetch(
        `https://api.flickr.com/services/rest/?method=flickr.photos.getRecent&api_key=${apiKey}&per_page=${perPage}&extras=views,description,date_upload,date_taken,owner_name,last_update,geo,url_o&format=json&nojsoncallback=1`
      );
